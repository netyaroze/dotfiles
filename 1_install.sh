#!/bin/bash
# Credit to: https://github.com/exah-io 
...
# Take in user input for the above variables:
# Ill leave it up to you to implement best practice for passwords.
printf '\033[33mPlease Enter The Installation Details:'
printf '\033[m \n' 
read -s -p "Enter Encryption Passphrase: " encryption_passphrase
printf "\n"
read -p "Enter a Username: " username
read -s -p "Enter a password for user $username: " user_password
printf "\n"
read -s -p "Enter a password for root user: " root_password
printf "\n"
read -p "Enter hostname: " hostname
read -p "Swap Size (In GB - typically size of ram if using hibernate): " swap_size
printf "\n"

# Confirm
printf '\033[33mParameters Defined'
printf '\033[m \n' 
echo "Username:" $username
echo "Hostname:" $hostname
echo "Swap Sizei (GB):" $swap_size 

# Set microcode, kernel params and initramfs modules 
cpu_microcode="intel-ucode"
kernel_options="i915.fastboot=1 i915.enable_fbc=1 i915.enable_guc=2"
initramfs_modules="intel_agp i915 nvidia nvidia_modeset nvidia_drm"
...
echo "Install is ready. You can safely reboot"
